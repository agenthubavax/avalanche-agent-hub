...
async function main() {
  ...
  // Initialize (if upgradeable)
  console.log("\n--- Initializing contracts ---");
  try {
    const initTx1 = await agentRegistry.initialize(deployer.address);
    await initTx1.wait();
    console.log("AgentRegistry initialized");
  } catch (e) {
    console.log("AgentRegistry already initialized or not upgradeable");
  }

  try {
    const initTx2 = await questFactory.initialize(deployer.address);
    await initTx2.wait();
    console.log("QuestFactory initialized");
  } catch (e) {
    console.log("QuestFactory already initialized or not upgradeable");
  }
